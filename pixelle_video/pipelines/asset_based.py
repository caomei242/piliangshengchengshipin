...
class AssetBasedPipeline(LinearVideoPipeline):
    ...
    async def produce_assets(self, context: PipelineContext) -> PipelineContext:
        ...
        for i, frame in enumerate(storyboard.frames, 1):
            ...
            tail_padding = float(context.request.get("narration_tail_padding", 0) or 0)
            if tail_padding > 0:
                padded_audio_path = Path(context.task_dir) / "frames" / f"{i:02d}_audio_padded.mp3"
                pad_cmd = [
                    'ffmpeg',
                    '-i', frame.audio_path,
                    '-f', 'lavfi',
                ...
                frame.audio_path = str(padded_audio_path)
                duration_cmd[-1] = frame.audio_path
                padded_duration_result = subprocess.run(
                    duration_cmd,
                    capture_output=True,
                    text=True,
                    check=True
                )
                frame.duration = float(padded_duration_result.stdout.strip())
                logger.info(f"Added {tail_padding:.2f}s narration tail padding to scene {i}")
