...
from pixelle_video.utils.gpu import ffmpeg_video_encoder_kwargs, log_gpu_status
...
class VideoService:
    ...
    def _ensure_ffmpeg(self):
        ...
        if not self._ffmpeg_checked:
            check_ffmpeg()
            log_gpu_status("ffmpeg")
            self._ffmpeg_checked = True

    def _video_encoder_kwargs(
        self,
        *,
        preset: str = "medium",
        crf: int = 23,
        ...
        return ffmpeg_video_encoder_kwargs(cpu_preset=preset, crf=crf, bitrate=bitrate)
            ...
            try:
                (
                    ...
                    .output(
                        ...
                        acodec='aac',
                        audio_bitrate='192k',
                        **self._video_encoder_kwargs(preset="medium", crf=23)
                    )
                    .overwrite_output()
                    .run(capture_stdout=True, capture_stderr=True)
                )
        ...
        try:
            ...
            (
                ...
                .output(
                    ...
                    acodec='aac',
                    audio_bitrate='192k',
                    **self._video_encoder_kwargs(preset="medium", crf=23, bitrate="2M")
                )
                .overwrite_output()
                .run(capture_stdout=True, capture_stderr=True)
            )
            ...
            if pad_strategy == "freeze":
                ...
                (
                    ...
                    .output(
                        video_stream,
                        output,
                        **self._video_encoder_kwargs(preset="fast", crf=23)
                    )
                    .overwrite_output()
                    .run(capture_stdout=True, capture_stderr=True, quiet=True)
                )
