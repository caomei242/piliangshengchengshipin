...
from pixelle_video.utils.gpu import chromium_gpu_args, log_gpu_status
...
class HTMLFrameGenerator:
    ...
    async def _ensure_browser(cls):
        ...
        if not browser_usable:
            ...
            log_gpu_status("playwright-chromium")
            cls._browser = await cls._playwright.chromium.launch(
                ...
                args=[
                    ...
                    '--disable-extensions',
                    *chromium_gpu_args(force_cpu=cls._force_cpu_browser),
                ]
            )
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    if self.__class__._force_cpu_browser:
                        raise
                    logger.warning("Chromium GPU screenshot failed; retrying with CPU rendering")
                    await page.close()
                    await self.__class__.close_browser()
                    self.__class__._force_cpu_browser = True
                    browser = await self._ensure_browser()
                    page = await browser.new_page(
                        viewport={'width': self.width, 'height': self.height},
                        device_scale_factor=1,
